...
- high_impact_claims: {{required_vendors, required_manufacturers, code_requirements, deal_breakers, evidence, validation_warnings}}
  IMPORTANT for required_manufacturers and APPROVED_MANUFACTURERS: Only list a manufacturer if it appears explicitly in a fire alarm context (e.g., in the fire alarm spec section, on a fire alarm schedule, next to panel/device references, or alongside NFPA 72). Do NOT list a manufacturer solely because the name appears on a hardware schedule, door hardware section, HVAC schedule, or general notes unrelated to fire alarm. For example, "Simplex" on a lock/door hardware schedule is NOT a fire alarm manufacturer requirement — only flag it if it appears on FA drawings, FA specifications, or panel/device schedules.
